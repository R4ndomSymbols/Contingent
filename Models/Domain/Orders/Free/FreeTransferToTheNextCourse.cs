...
public class FreeTransferToTheNextCourseOrder : FreeEducationOrder
{
    ...
    internal override async Task<Result<bool>> CheckConductionPossibility()
    {
        ...
        foreach(var move in _moves.Moves){
            ...
            if (currentGroup is null || !currentGroup.SponsorshipType.IsFree() || currentGroup.CourseOn == currentGroup.EducationProgram.CourseCount){
                return Result<bool>.Failure(new ValidationError(nameof(_moves), "Студент числится в группе, для которой невозможно проведение данного приказа"));
            }
            ...
            if (!targetGroup.SponsorshipType.IsFree() || targetGroup.CourseOn - currentGroup.CourseOn != 1 || currentGroup.HistoricalSequenceId != targetGroup.HistoricalSequenceId){
                return Result<bool>.Failure(new ValidationError(nameof(_moves), "Текущая группа и целевая группа несовместны в рамках данного приказа"));
            }    
        }
