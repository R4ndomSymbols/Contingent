...
public class StudentModel
{
    ...
    public AddressModel? ActualAddress {
        get {
            if (_actualAddressId != Utils.INVALID_ID && _actualAddress is null){
        ...
        set {
            if (value is null){
                throw new Exception("Нельзя присвоить студенту несуществующий адрес");
            }
    ...
    public async Task<ResultWithoutValue> Save(ObservableTransaction? scope) 
    {
        ...
        cmd.Parameters.Add(new NpgsqlParameter("p3", ActualAddressId == null ? DBNull.Value : ActualAddressId.Value));
    ...
    public async Task Update(ObservableTransaction? scope = null)
    {
        ...
        cmd.Parameters.Add(new NpgsqlParameter<string>("p1", _snils));
        cmd.Parameters.Add(new NpgsqlParameter<string>("p2", _inn));
        cmd.Parameters.Add(new NpgsqlParameter("p3", (int)_actualAddress.Id));
        cmd.Parameters.Add(new NpgsqlParameter<DateTime>("p4", _dateOfBirth));
